<?php
	...
	if(isset($_GET['id']))
	{
		$rs=mysqli_query($conn, "Select * from mata_kuliah where sha1(kode_mata_kuliah)='".$_GET['id']."'");
		$row=mysqli_fetch_array($rs);
?>
		<form name="form1" method="post" action="?cat=akademik&page=editmatkul&id=<?php echo $_GET['id']; ?>&edit=1">
		  <table width="50%" border="0" cellspacing="0" cellpadding="0">
			<tr>
			  <td width="36%">Nama Mata Kuliah</td>
			  <td width="64%"><label for="Mata Kuliah"></label>
...
if(isset($_GET['edit']))
{
	$rs=mysqli_query($conn, "Update mata_kuliah SET nama_mata_kuliah='".$_POST['jenis']."' Where sha1(kode_mata_kuliah)='".$_GET['id']."'");
	if($rs)
	{
		echo "<script>window.location='?cat=akademik&page=matkul'</script>";
	}
