...
<span class="span5">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-striped">
	  ...
	  <?php
		  $rw=mysqli_query($conn, "Select * from mata_kuliah");
...
if(isset($_GET['act']))
{
	$rs=mysqli_query($conn, "Insert into mata_kuliah (`nama_mata_kuliah`) values ('".$_POST['nama_mata_kuliah']."')") or die(mysqli_error());
	if($rs)
	{
...
{
	$ids=$_GET['id'];
	$ff=mysqli_query($conn, "Delete from mata_kuliah Where sha1(kode_mata_kuliah)='".$ids."'");
	if($ff)
	{
		echo "<script>window.location='?cat=akademik&page=matkul'</script>";
	}
